...
type ProjectItemType = ILanguageTexts["projects"]["projectList"][0];
        ...
        <Link href={`/projects/${project.id}`} className="block cursor-pointer">
            <div className="mb-4">
                ...
                <ul className="list-disc list-inside mt-2 text-amber-300">
                    {project.functionality.map((func: string, index: number) => (
                ...
                <p className="mt-2 text-3xs text-green-700">
                    {project.technologies.map((tech: { name: string }) => tech.name).join(', ')}
                </p>
